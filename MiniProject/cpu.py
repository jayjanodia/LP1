...
def cpu_convert_frame(image):
    ...
    yuv[:, :, 1] = -0.14713 * r_channel - 0.28886 * g_channel + 0.436 * b_channel + 128
    yuv[:, :, 2] = 0.615 * r_channel - 0.51499 * g_channel - 0.10001 * b_channel + 128
    return yuv
...
def convert_videos_cpu(input_file, output_file):
    ...
    out = cv2.VideoWriter('cpu_' + output_file, fourcc, 25.0, (frame_width, frame_height))
    while(cap.isOpened()):
