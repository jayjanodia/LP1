# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 21:39:35 2019
...
def main():
    ...
    parser.add_argument('-hc', help='Select CPU for HD processing', action='store_true')
    parser.add_argument('-g', help='Select GPU for processing', action='store_true')
    parser.add_argument('-hg', help='Select GPU for HD processing', action='store_true')
    ...
    try:
        if(args.i == True and args.s != None and args.d != None):
            ...
            if(args.i and args.hc):
                cpu1.convert_images_cpu(args.s, args.d)
            if(args.i and args.hg):
                gpu1.convert_images_gpu(args.s, args.d)
        elif(args.i == True):
        ...
        if(args.v == True and args.input != None and args.output != None):
            if(args.v and args.c):
                cpu.convert_videos_cpu(args.input, args.output)
            if(args.v and args.g):
                gpu.convert_video_gpu(args.input, args.output)
            if(args.v and args.hc):
                cpu1.convert_videos_cpu(args.input, args.output)
            if(args.v and args.hg):
                gpu1.convert_video_gpu(args.input, args.output)
        elif(args.v == True):
