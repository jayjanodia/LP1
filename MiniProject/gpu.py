# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 21:17:05 2019
...
def ret_u_channel(r_channel, g_channel, b_channel):
    return -0.14713 * r_channel - 0.28886 * g_channel + 0.436 * b_channel + 128

@vectorize(['float32(float32, float32, float32)'], target = 'cuda')
def ret_v_channel(r_channel, g_channel, b_channel):
    return 0.615 * r_channel - 0.51499 * g_channel - 0.10001 * b_channel + 128
...
def convert_images_gpu(source_dir, destination_dir):
    ...
    for file_name in list_of_files:
        try:
            ...
            cv2.imwrite(destination_dir + 'gpu_' + file_name, ret_image)
...
def convert_video_gpu(input_file, output_file):
    ...
    out = cv2.VideoWriter('gpu_' + output_file, fourcc, 25.0, (frame_width, frame_height))
    while(cap.isOpened()):
